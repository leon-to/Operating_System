...
/*
 * Command for changing debug flags.
 */
static
int
cmd_dbflags(int nargs, char **args)
{
	...
	//hex array
	int hex_array[12]={0x001, 0x002, 0x004, 0x008,
			   0x010, 0x020, 0x040, 0x080, 
			   0x100, 0x200, 0x400, 0x800};

	char *option          ;
	int  flag,    hex_flag,    old_flag; 
	//flag: new flag, hex_flag: new hex flag, old_flag: old hex flag 

	option   = args[2];
	flag     = atoi( args[1] );
	hex_flag = hex_array[ flag-1 ];

	//Flag Algorithm: 1. Save old flagg from dbflags
	//		  2. AND dbflags to erase old flag from dbflags 
	//		  3. Plug in or out new flag to dbflags by ORing dbflags
	if     ( (1<=flag) &  (flag<=4) ){
		old_flag  = dbflags & 0x00f;
		dbflags  &= 0xff0;
	}
	else if( (5<=flag) & (flag<=8)){
		old_flag  = dbflags & 0x0f0;
		dbflags &= 0xf0f;
	}
	else if( (9<=flag) & (flag<=12)){
		old_flag  = dbflags & 0xf00;
		dbflags &= 0x0ff;
	}			
	else{
		kprintf("Usage: df nr on/off\n");
	...
	//ON/OFF option. (ORing dbflags in step 3) 
	if( strcmp(option, "on") == 0 ){
		dbflags |= hex_flag;
	}
	else if( strcmp(option, "off") == 0){
		if( hex_flag != old_flag ) 
			dbflags |= old_flag;
	}
	else{
		kprintf("Usage: df nr on/off\n");
...
static struct {
	...
	{ "exit",	cmd_quit },
	{ "halt",	cmd_quit },
	{ "df",         cmd_dbflags },//add new operation: dbflags 
